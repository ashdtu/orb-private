...
MapPublisher::MapPublisher(Map* pMap):mpMap(pMap), mbCameraUpdated(false)
{
    ...
    pointcloud_pub=nh.advertise<sensor_msgs::PointCloud2>("ORB_SLAM/Pointcloud",1);
...
void MapPublisher::CloudPublisher(const vector<MapPoint*> &vpMPs)
{
    vector<MapPoint*> point_cloud=mpMap->GetAllMapPoints();
    ...
    msg->fields.resize(3);
    msg->fields[0].name = "x";
    msg->fields[0].offset = 0*sizeof(uint32_t);
    msg->fields[0].datatype = sensor_msgs::PointField::FLOAT32;
    msg->fields[0].count = 1;
    msg->fields[1].name = "y";
    msg->fields[1].offset = 1*sizeof(uint32_t);
    msg->fields[1].datatype = sensor_msgs::PointField::FLOAT32;
    msg->fields[1].count = 1;
    msg->fields[2].name = "z";
    msg->fields[2].offset = 2*sizeof(uint32_t);
    msg->fields[2].datatype = sensor_msgs::PointField::FLOAT32;
    msg->fields[2].count = 1;

    msg->point_step = 3*sizeof(uint32_t);
    msg->row_step = msg->point_step * msg->width;
    msg->data.resize(msg->row_step * msg->height);
    msg->is_dense = false;

    unsigned char* dat = &(msg->data[0]);
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend;i++)
    {
        ...
        //geometry_msgs::Point p;
        cv::Mat pos = vpMPs[i]->GetWorldPos();
        /*msg->points[i].x=pos.at<float>(0);
        msg->points[i].y=pos.at<float>(1);
        msg->points[i].z=pos.at<float>(2);*/
        memcpy(dat, &(pos),3*sizeof(float));
        dat+=msg->point_step;

    }

        //msg->points.push_back(pcl::PointXYZ(msg));
